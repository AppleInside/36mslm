...
- **Giornalisti e stampa locale** — accedono al sito principalmente per materiale multimediale degli eventi (foto watermarkate, richiesta alta-res via press kit).
...
1. **Promozione territoriale.** Ogni sezione serve a far conoscere e visitare il territorio: itinerari, ricette, storia, eventi sono strumenti di promozione, non fini a sé stessi.
2. **Organizzazione eventi.** Il sito è lo strumento operativo dell'associazione per comunicare, gestire le iscrizioni e documentare gli eventi.
3. **Tutela del territorio.** Valorizzazione del paesaggio e del patrimonio naturale e culturale, con una presenza istituzionale credibile come APS regolarmente iscritta.
4. **Semplicità gestionale.** Niente CMS complesso, niente admin per non-tecnici. I contenuti vivono in MDX nel repo, gestiti da una sola persona tecnica.
5. **Bilinguismo IT/EN graduale.** IT è la lingua primaria; EN si traduce file per file con calma, senza auto-translation.
6. **Tutela diritti foto by default.** Watermark automatico su tutte le foto pubbliche; gli originali restano in bucket privato.
